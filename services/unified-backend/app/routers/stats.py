...
@router.get("", response_model=SystemStats, summary="Get system-wide statistics")
async def get_stats(
    request: Request,
    ...
    return SystemStats(
        documents=DocumentStats(total=doc_total, by_status=doc_by_status),
        chunks=ChunkStats(
            total=chunk_total,
        ...
        bm25=BM25Stats(index_size=bm25_index_size),
    )
